...
cores_classes = {
    ...
    "Áreas antropizadas": "#e73e0b", # laranja
    "Áreas urbanizadas": "#808080",  # cinza
...
def style_function(feature):
    ...
    cor = cores_classes.get(classe, "#FFFFFF")  # cor padrão: preto
    return {
        'fillColor': cor,
        'color': cor,
        'weight': 1,
...
def criar_mapa():
    ...
    ">
        ...
        <div><span style="background:#e73e0b; width: 20px; height: 20px; display: inline-block; margin-right: 8px;"></span>Áreas antropizadas</div>
    ...
    script_sync_mapas = folium.Element("""
    <script>
    ...
    window.addEventListener('message', function(event) {
        ...
        try {
            if (!event.data || event.data.type !== 'centralizarMapa') return;

            var maps = Object.values(window).filter(v => v instanceof L.Map);
            if (maps.length === 0) return;

            var map = maps[0];
            var boundsStr = event.data.bounds;
            var nomeBacia = event.data.nome;

            if (!boundsStr || !nomeBacia) return;

            var parts = boundsStr.split(',').map(parseFloat);
            if (parts.length === 4) {
                // Esperado: [south, west, north, east]
                var south = parts[0],
                    west = parts[1],
                    north = parts[2],
                    east = parts[3];
                ...
                console.log("Centralizando para bounds:", bounds);
            ...
            map.eachLayer(function(layer) {
                if (layer.feature && layer.feature.properties && layer.feature.properties.NOME === nomeBacia) {
            ...
            console.error('Erro ao centralizar e destacar bacia:', err);
        }
    });
    </script>
    """)
    ...
    mapa.get_root().html.add_child(script_sync_mapas)
...
caminho_saida = os.path.join(os.path.dirname(__file__), '..', 'pg_conheca', '03_mapa_pg_conheca_uso.html')
