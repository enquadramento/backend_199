...
def escolas1(x):
    geojson = read_file("data/geojson/07_ESCOLAS_MBH.geojson")

    # Cores e ícones por tipo de escola
    estilo_escolas = {
        'Estadual': {'color': 'blue',    'icon': 'graduation-cap'},
        'Federal':  {'color': 'green',   'icon': 'university'},
        'Municipal':{'color': 'orange',  'icon': 'school'},
        'Privada':  {'color': 'red',     'icon': 'user-graduate'}
    }
    ...
    for _, row in geojson.iterrows():
        ...
        estilo = estilo_escolas.get(tipo, {'color': 'gray', 'icon': 'question'})
...
def criar_mapa():
    ...
    legenda = folium.Element("""
        ...
        <div style="position: fixed; 
            ...
            <b>Tipos de Escola</b><br>
            
            <i class="fa fa-graduation-cap" style="color:#3399FF"></i> Estadual<br>
            <i class="fa fa-university" style="color:green"></i> Federal<br>
            <i class="fa fa-school" style="color:orange"></i> Municipal<br>
            <i class="fa fa-user-graduate" style="color:red"></i> Privada
        </div>
        """)
...
caminho_saida = os.path.join(os.path.dirname(__file__), '..', 'pg_panorama', '03_mapa_interativo_escolar.html')
